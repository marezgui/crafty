...
export class FileSystemFolloweeRepository implements FolloweeRepository {
  ...
  async saveFollowee({ user, followee }: Followee): Promise<void> {
    const followees = await this.getFollowees();
    const actualUserFollowees = followees[user] ?? [];
    actualUserFollowees.push(followee);
    followees[user] = actualUserFollowees;
